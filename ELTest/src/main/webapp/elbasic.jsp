<%@page import="java.util.LinkedList"%>
<%@page import="hello.DataBean"%>
<%@page import="java.util.List"%>
<%@page import="java.util.ArrayList"%>
<%@page import="java.util.Map"%>
<%@page import="java.util.HashMap"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>

<!--  EL Expression Syntax -->

<!--  문자열 출력 -->
${'Hello EL ~!'}${"EL"}<br />
<%
	// _jspService 메소드의 지역변수
	String hello = "HelloHello";

	// JSP에서 스크립트릿을 사용해서 pageContext 영역객체에 hello라는 속성변수 명으로 "Hello Hello Hello" 속성값을 저장
	pageContext.setAttribute("hello","Hello Hello");
%>

<!--  기본적으로 EL에서 변수는 pageScope 부터 찾기 때문에 아래는 pageScope.hello와 같다 -->
${hello}<br/>
<%-- \${pageContext.hello}<br/> --%>
${pageScope.hello}<br/>
${hello==pageScope.hello}

<!--  아래의 anyVariable이라는 변수는 pageScope.anyVariable과 같다
	  변수가 없어도 (혹은 null)이라도 EL에서는 굳이 익셉션이나 에러를 발생시키지 않는다
 -->
 ${anyVariable}
 
 <%
 	//request 속성 변수 생성
	hello.HelloBean helloBean = new hello.HelloBean();
 	helloBean.setStr("Hello~!");
	request.setAttribute("var1", helloBean);
 %>
 <!--  requestScope에서 속성변수 var1의 값을 출력 -->
 ${requestScope.var1}<br/>
 <!--  var1 객체의 str 프라퍼티의 값을 출력하려면 var1 객체에 public getter 메소드가 있어야 함 -->
 \${requestScope.var1.str}<br/>
 
 <!-- EL의 데이터 타입 --><br/>
 ${false}
 <br/>
 ${10}
 <br/>
 ${3.3}
 ${'hello'}${"hello"}
 <br/>
 ${null}
 <!--EL의 기본객체 -->
 <br /> <br/>
 <!-- pageContext객체를 통해 다른 기본객체들을 얻어올 수 있다. 예를 들어.. request, session, servletContext... -->
 ${pageContext.request}<br />
 <!-- 각 영역에 대한 영역객체 (각 영역의 속성변수들을 설정하거나 가져온다) -->
 ${pageScope} <br/>
 ${requestScope} <br/>
 ${sessionScope} <br/>
 ${applicationScope} <br/>
 
 <!--  request 파라미터들에 대한 객체 -->
 <!--  request.getParameter(파라미터 변수명) -->
 ${param} <br/>
 	<!-- request 파라미터의 값들만 -->
 ${paramValues} <br/>
 <!-- request 헤더들에 대한 객체 -->
 ${header} <br/>
 ${headerValues} <br/>
 
 <!-- cookie 객체 -->
 ${cookie}<br/>
 
 <!-- application(servletContext)의 초기화 파라미터  -->
 ${initParam.name}<br/>
 
 <!-- 객체 접근 -->
  <%
  	DataBean dataBean = new DataBean();
  
  	Map<String, Object> map = new HashMap<String, Object>();
  	dataBean.setMap(map);
  	map.put("map1", "map1Value");
  	map.put("map2", "map2Value");
  	map.put("map3", null);
  	
  	List<String> list = new ArrayList<String>();
  	dataBean.setList(list);
  	list.add("list1");
  	list.add("list2");
  	list.add(null);
  	
  	String[] strArray = {"hello", "EL", "world",null};
  	dataBean.setStrArray(strArray);
  	
  	pageContext.setAttribute("dataBean", dataBean);
  %>
 ${dataBean.map.map1}<br/>
 ${dataBean.list[0]}<br/>
 ${dataBean.strArray[0]}<br/>
 
 <!-- Map이나 List 객체 또는 배열-->
  ${dataBean.map.map3}<br/>
 ${dataBean.list[2]}<br/>
 ${dataBean.strArray[3]}<br/>
 
<!--   Map이나 List 객체 또는 배열이 null일 때 -->
<!--   EL에서는 null에 대한 처리를 내부적으로 수행하지만 화면에는 아무것도 출력하지 않도록 설계되어 있다 -->


 <%
//  	Map<String, Object> map2 = null;
 	List<String> list2 = null;
 	String[] strArray2 = null;
 	
//  	pageContext.setAttribute("map2", map2); 	
 	pageContext.setAttribute("list2", list2);
 	pageContext.setAttribute("strArray2", strArray2);
 %>
 
 map2 : ${map2}<br/>
 list2 : ${list2}<br/>
 strArray2 : ${strArray2}<br/><br/>
 
 <!--  1, +, -, * -->
 <%
 	pageContext.setAttribute("nullValue",null);
 	pageContext.setAttribute("longValue",new Long(100));
 	pageContext.setAttribute("floatValue",new Float(3.3));
 %>
 <br/><br/>
 ${pageScope.longValue+pageScope.nullValue}<br/>
 ${pageScope.nullValue+pageScope.longValue}<br/>
 ${pageScope.floatValue+pageScope.nullValue}<br/>
 ${pageScope.nullValue+pageScope.floatValue}<br/>
 <br/><br/>
 ${100+pageScope.nullValue}<br/>
 ${pageScope.nullValue+100}<br/>
 ${3.3+pageScope.nullValue}<br/>
 ${pageScope.nullValue+3.3}<br/>

<!-- 2. /, %, -(단항) -->
<br/><br/>
 \${pageScope.longValue/pageScope.nullValue}<br/>
 145 : ${pageScope.nullValue/pageScope.longValue}<br/>
 \${pageScope.floatValue/pageScope.nullValue}<br/>
 147 : ${pageScope.nullValue/pageScope.floatValue}<br/>
 <br/><br/>
 149 : ${100/pageScope.nullValue}<br/>
 150 : ${pageScope.nullValue/100}<br/>
 151 : ${3.3/pageScope.nullValue}<br/>
 152 : ${pageScope.nullValue/3.3}<br/>
 <br/><br/>
 \${pageScope.longValue%pageScope.nullValue}<br/>
 155 : ${pageScope.nullValue%pageScope.longValue}<br/>
 \${pageScope.floatValue%pageScope.nullValue}<br/>
 157 : ${pageScope.nullValue%pageScope.floatValue}<br/>
 <br/><br/>
 \${100%pageScope.nullValue}<br/>
 160 : ${pageScope.nullValue%100}<br/>
 \${3.3%pageScope.nullValue}<br/>
 162 : {pageScope.nullValue%3.3}<br/>
 <br/><br/>
 ${-pageScope.nullValue}<br/>
 ${-pageScope.longValue}<br/>
 ${-pageScope.floatValue}<br/>
 ${-0}<br/>
 ${-100}<br/>
 ${-3.3}<br/>
 
 <!-- 비교연산자 -->
 <%
 	pageContext.setAttribute("value1",20);
 	pageContext.setAttribute("value2",20);
 %>
 ${value1 > value2} or ${value1 gt value2}<br/>
 ${value1 < value2} or ${value1 lt value2}<br/>
 ${value1 >= value2} or ${value1 ge value2}<br/>
 ${value1 <= value2} or ${value1 le value2}<br/>
 ${value1 == value2} or ${value1 eq value2}<br/>
 ${value1 != value2} or ${value1 ne value2}<br/>
 
 <!-- 논리연산자 -->
 ${true && false} or ${true and false}<br />
 ${true || false} or ${true or false}<br />
 ${!true} or ${not true}
 
 <!-- empty 연산자 -->
 <%
 	Object obj1 = new Object();
 	Object obj2 = null;
 	Map<String, Object> map1 = new HashMap<String,Object>();
 	Map<String, Object> map2 = null;
 	Map<String, Object> map3 = new HashMap<String,Object>();
 	map3.put("entryKey","entryValue");
 	
 	List<String> list1 = new LinkedList<String>();
 	List<String> list3 = null;
 	List<String> list4 = new LinkedList<String>();
 	list4.add("item");
 	
 	String[] strArray1 = null;
 	String[] strArray3 = new String[10];
 	String[] strArray4 = new String[10];
 	strArray4[0] = "item";
 	
 	pageContext.setAttribute("obj1",obj1);
 	pageContext.setAttribute("obj2",obj2);
 	pageContext.setAttribute("map1",map1);
 	pageContext.setAttribute("map2",map2);
 	pageContext.setAttribute("map3",map3);
 	pageContext.setAttribute("list1",list1);
 	pageContext.setAttribute("list3",list3);
 	pageContext.setAttribute("list4",list4);
 	pageContext.setAttribute("strArray1",strArray1);
 	pageContext.setAttribute("strArray3",strArray3);
 	pageContext.setAttribute("strArray4",strArray4);
 %>
 
 
 <br/><br/>
 ${empty obj1}<br/>
 ${empty obj2}<br/>
 ${empty map1}<br/>
 ${empty map2}<br/>
 ${empty map3}<br/>
 ${empty list1}<br/>
 ${empty list3}<br/>
 ${empty list4}<br/>
 ${empty strArray1}<br/>
 ${empty strArray3}<br/>
 ${empty strArray4}<br/>
 
 <!-- 3항 연산자 -->
 
 ${3==3 ? "3은 3입니다" : "3은 3이 아닙니다"} ><br/>
 ${empty map3 ? "map3은 엔트리가 없습니다" : "map3은 엔트리가 있습니다"}<br/>
 !${empty map3 ? "map3은 엔트리가 없습니다" : "map3은 엔트리가 있습니다"}
 
 <!-- 문자열 연결 -->
 ${"hello" + "world"}<br />
 ${"hello" +="world"}<br />	